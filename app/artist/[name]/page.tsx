    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-800">
        <div className="text-center">
          ...
          <p className="mb-4">Maaf, kami tidak menemukan seniman dengan nama "{artistName}".</p>
    ...
    <div className="min-h-screen bg-white text-slate-900">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
            <div className="flex-1">
              <h1 className="text-5xl font-bold mb-2 tracking-tight">{artist.name}</h1>
              <p className="text-xl text-slate-400 mb-4">{artist.years} • {artist.nationality || 'Unknown'}</p>
              
              {/* Extra metadata returned from action.ts */}
              <div className="flex flex-wrap gap-4 mb-6">
                <span className="text-sm text-slate-300">Total karya: <strong className="text-white ml-1">{artist.paintings_count ?? '—'}</strong></span>
                {artist.school && (
                  <span className="text-sm text-slate-300">School: <strong className="text-white ml-1">{artist.school}</strong></span>
                )}
              </div>

              <div className="flex flex-wrap gap-2 mb-6">
                {artist.movements.map((m: string) => (
                  <span key={m} className="bg-blue-600/20 text-blue-300 border border-blue-600/30 px-3 py-1 rounded-full text-sm">
                    {m}
                  </span>
                ))}
              </div>
              ...
              <div className="prose prose-invert max-w-none">
                <p className="text-slate-300 leading-relaxed text-lg">{artist.bio}</p>
              </div>
              
              {artist.wikipedia && artist.wikipedia !== '#' && (
                <a href={artist.wikipedia} target="_blank" rel="noopener noreferrer" className="mt-4 inline-block text-blue-400 hover:text-blue-300 text-sm">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {artist.artworks.map((work: any, idx: number) => (
            <div key={idx} className="group">
              ...
              <p className="text-xs text-slate-500 mt-1 truncate">{work.info}</p>
            </div>
          ))}
        </div>
        
        {artist.artworks.length === 0 && (
          <p className="text-slate-500 italic text-center py-10">Belum ada data gambar untuk seniman ini.</p>
        )}
      </div>
    </div>
  );
