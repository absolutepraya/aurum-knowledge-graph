...
export default function ConsolePage() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setIsError(true);
      setResult(`Terjadi kesalahan sistem: ${e?.message ?? String(e)}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={`min-h-screen bg-slate-900 text-slate-200 p-8 font-mono transition-all duration-[1200ms] ease-in-out ${entered ? 'opacity-100 translate-y-0 scale-100 blur-0' : 'opacity-0 translate-y-4 scale-95 blur-sm'}`}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-black rounded border border-slate-700 p-4 overflow-x-auto">
          <div className="flex justify-between mb-2 border-b border-slate-800 pb-2">
            ...
            {isError && <span className="text-xs text-red-500 font-bold">FAILED</span>}
            {!isError && !isLoading && result && <span className="text-xs text-green-500 font-bold">SUCCESS</span>}
          </div>
          <pre className={`text-sm ${isError ? 'text-red-400' : 'text-slate-300'}`}>
        ...
        <div className="mt-8 p-4 bg-slate-800/50 rounded text-xs text-slate-400">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li><code>MATCH (a:Artist) RETURN a.name, a.nationality LIMIT 5</code></li>
            <li><code>MATCH (a:Artist)-[:CREATED]-&gt;(w:Artwork) WHERE a.name CONTAINS 'Vinci' RETURN w.title</code></li>
            <li><code>MATCH (n) RETURN labels(n) as Label, count(n) as Total</code></li>
          </ul>
        </div>
      </div>
    </div>
