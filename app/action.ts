 ...
 */
export async function searchGlobal(keyword: string): Promise<GlobalSearchResult[]> {
...
export async function getArtistDetail(artistName: string): Promise<ArtistDetail | null> {
  ...
  try {
    ...
    const artworks = record.get('artworks').filter((art: any) => art.title); // Filter yang kosong jika ada
...
export async function executeCypherQuery(cypherQuery: string): Promise<any> {
  ...
  try {
        ...
        record.keys.forEach((key: string, index: number) => {
            ...
            if (value && typeof value === 'object') {
                if (value.properties) {
                    ...
                    value = { ...value.properties, _id: value.identity?.toString(), _labels: value.labels || value.type };
    ...
    return { 
        error: true, 
        message: error.message || 'Terjadi error saat menjalankan query.'
    };
  } finally {
    await session.close();
  }
